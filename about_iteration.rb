...
class AboutIteration < Neo::Koan
  ...
  def test_iterating_with_each
    array = [1, 2, 3]
    sum = 0
    array.each do |item| # item is 1, item is 2, item is 3
      sum += item
    end
    assert_equal 6, sum
  end
  ...
  def test_each_can_use_curly_brace_blocks_too
    array = [1, 2, 3]
    sum = 0
    array.each { |item| sum += item } # item is 1, item is 2, item is 3
    assert_equal 6, sum
  end
  ...
  def test_break_works_with_each_style_iterations
    ...
    sum = 0
    array.each do |item| #for each item
      break if item > 3 # stop when item is greater than three
      sum += item
    end
    assert_equal 6, sum
  end
  ...
  def test_inject_will_blow_your_mind
    result = [2, 3, 4].inject(0) { |sum, item| sum + item } #perform the operation outside the pipe
    #take each elements and adds them together
    ...
    #inject performs operation on all elements together
  end
